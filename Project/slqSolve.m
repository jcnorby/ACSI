function [x_bar,K,u_bar] = slqSolve(x_bar,u_bar,N,dt,x0)
u_ff = ones(size(u_bar));

ii = 1;
while (norm(u_ff)>=1e-6) && ii <=10
    % Initialize S matrix (coefficients of quadratic functional)
    s(N+1) = compute_qN(x_bar(:,N+1));
    s_vec(:,N+1) = compute_qN_vec(x_bar(:,N+1));
    S_mat(:,:,N+1) = compute_QN_mat(x_bar(:,N+1));
